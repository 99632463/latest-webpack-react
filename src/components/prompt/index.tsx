...
class Prompt extends React.Component<IPromptProps, IPromptState> {
  ...
  render() {
    ...
    return (
      <div className={`toast toast-fixed ${this.state.showPrompt ? 'slide-show' : ''}`} ref={this.node}>
        <div className="toast-header">
          <img src="../sources/images/no-attachment.png" className="rounded mr-2" width="15px" />
          <strong className="text-dark mr-auto">{title || ''}</strong>
          <button type="button" className="ml-2 mb-1 close">
            <span aria-hidden="true" onClick={() => this.close()}>×</span>
          </button>
        </div>
